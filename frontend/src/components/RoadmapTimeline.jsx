...
export default function RoadmapTimeline({ roadmap = [], onToggleTask, isUpdatingProgress }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
              ...
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Personalized Action Roadmap</h3>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-main)', fontFamily: 'var(--font-mono)' }}>
                {completedCount} <span style={{ fontSize: '0.9rem', color: 'var(--text-dim)' }}>/ {roadmap.length}</span> ({completionRate}%)
              </div>
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
        {Object.entries(phases).map(([phaseHeader, tasks], phaseIdx) => (
          <div key={phaseIdx} className="glass-panel" style={{ padding: '22px 24px' }}>
            <div style={{ 
              fontSize: '0.8rem', color: 'var(--accent-secondary)', fontWeight: 700, 
              ...
              {tasks.map(task => {
                ...
                return (
                  <div 
                    key={task.id}
                    style={{
                      background: task.is_completed ? 'rgba(16, 185, 129, 0.06)' : 'rgba(255, 255, 255, 0.02)',
                      border: task.is_completed ? '1px solid rgba(16, 185, 129, 0.3)' : '1px solid var(--card-border)',
                      borderRadius: 'var(--radius-md)',
                      padding: '16px 20px',
                      transition: 'all var(--transition-normal)'
                    }}
                  >
                    {/* Top Row: Checkbox, Week, Title, Hours, Expand */}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flex: 1 }}>
                        <button
                          ...
                          style={{
                            ...
                            border: 'none',
                            cursor: 'pointer',
                            color: task.is_completed ? 'var(--accent-emerald)' : 'var(--text-dim)',
                            display: 'flex',
                            alignItems: 'center',
                            padding: 0,
                    ...
                    {isExpanded && (
                      <div style={{ 
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                          ...
                          <div>
                            <strong style={{ color: '#ffffff' }}>Why it matters:</strong>{' '}
                            <span style={{ color: 'var(--text-secondary)' }}>{task.why_it_matters}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                          ...
                          <div>
                            <strong style={{ color: '#ffffff' }}>Objective:</strong>{' '}
                            <span style={{ color: 'var(--text-secondary)' }}>{task.learning_objective}</span>
                          </div>
                        </div>
                        ...
                        <div style={{ 
                          ...
                          <div>
                            <div style={{ fontWeight: 700, color: 'var(--accent-secondary)', fontSize: '0.74rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '4px' }}>
                              Practical Deliverable
                            </div>
                            <div style={{ color: 'var(--text-main)', fontFamily: 'var(--font-mono)', fontSize: '0.82rem', lineHeight: 1.5 }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', color: 'var(--text-muted)', fontSize: '0.78rem' }}>
                          ...
                          <div>
                            <strong>Verification Standard:</strong> {task.completion_criteria}
                          </div>
                        </div>
                      </div>
                    )}
